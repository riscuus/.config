...
# Function to show the current Git branch and color it red if there are uncommitted changes
function git_branch {
  ...
  if [ $? -eq 0 ]; then
    ...
    if [ $? -eq 0 ]; then
      ...
      # If there are no changes, print the branch normally
      echo "[$branch]"
    fi
  fi
}
...
cd /mnt/c/NASA/firmware_dec_24/firmware/gitlab/
